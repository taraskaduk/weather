...
ggplot() + 
  geom_point(data = stations_import, aes(x = lon, y = lat), alpha = 0.1, col = 'red') +
...
stations_needed <- cities_stations %>% 
  select(station) %>% 
  mutate(station = paste0(station,".csv")) %>% 
  distinct() %>% .[ , "station"]


files_remove <- subset(files_all, !(files_all %in% stations_needed))
files_missing <- subset(stations_needed, !(stations_needed %in% files_all))
